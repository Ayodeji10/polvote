      ...
      <div className="body">
        <div className="row header justify-content-between align-items-center">
          <div className="col-6">
            ...
            <p className="mb-0">
              {live ? liveVotes.toFixed(0) : pollVotes.toFixed(0)} Total Votes
            </p>
          </div>
          <div className="col-5 d-flex align-items-center">
            ...
            <h4 className="mb-0" onClick={() => setLive(!live)}>
              {live ? "Switch to Polvote Result" : "Switch to Election Result"}
            </h4>
          </div>
        </div>
        {live ? (
          <>
            <h6 id="startdate">
              Election Date is{" "}
              {`${poll.livevotedate.substring(
                8,
                10
              )}-${poll.livevotedate.substring(
                5,
                7
              )}-${poll.livevotedate.substring(0, 4)}`}
            </h6>
            {poll.aspirant
              ...
              .map((aspirant, index) => {
                return (
                  <OpenPollCandidate
                    ...
                    fetchcurrentPollAndParties={fetchcurrentPollAndParties}
                  />
                );
              })}
          </>
