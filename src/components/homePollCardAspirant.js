...
function HomePollCardAspirant({
  ...
  const checkVote = () => {
    if (localStorage.getItem("ballotbox_token") === null) {
      ...
      const filteredVotes = currentPoll.aspirant.filter(
        (aspirant) =>
  ...
  const vote = (aspirantId) => {
    const aspirant = currentPoll.aspirant.filter(
      ...
      <div className="row align-items-center">
        ...
        <div className="col-lg-7 col-md-7 col-sm-7 col-6">
          ...
          <div className="bar">
            <div
              className="indicator"
              style={{ width: `${(aspirant.votes.length / pollToTal) * 100}%` }}
            />
          </div>
        </div>
        <div className="col-lg-3 col-md-3 col-sm-3 col-4 d-flex justify-content-between align-items-center">
          ...
          <button
            ...
            onClick={checkVote}
          >
            {aspirant.votes.filter((vote) => vote === context.user._id).length >
