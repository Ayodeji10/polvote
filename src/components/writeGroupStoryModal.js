...
function WriteGroupStoryModal({ writeStoryModal, setWriteStoryModal, group }) {
  // context
  ...
  const writeStory = () => {
    ...
    fd.append("post", storyText);
    for (const key of Object.keys(images)) {
      fd.append("image", images[key]);
    }
    // console.log(Array.from(fd))
    axios({
      url: `${API.API_ROOT}/post/addpost`,
      method: "POST",
      headers: {
  ...
  return (
    <Modal
      isOpen={writeStoryModal}
      onRequestClose={() => setWriteStoryModal(false)}
      className={`story-write-modal ${context.darkMode ? "dm" : ""}`}
    >
      <i
        className="far fa-times-circle"
        onClick={() => setWriteStoryModal(false)}
      />
      <h2 className="header">New Group Post</h2>
      ...
      <div className="d-flex justify-content-between align-items-center">
        ...
        <div>
          ...
          {storyLoading ? (
            <button id="post" disabled>
              <>
                Loading... <i className="fa-solid fa-spinner fa-spin" />
              </>
            </button>
          ) : (
            <button id="post" onClick={writeStory}>
              Post
            </button>
          )}
        </div>
...
export default WriteGroupStoryModal;
