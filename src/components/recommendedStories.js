...
function RecommendedStories() {
    ...
    return (
        <div className="story-recomentdations mb-3">
            ...
            {stories.slice(0).sort(function () { return .5 - Math.random() }).slice(0, 5).map((each, index) => { ///slice(0) at the beginning is to duplicate the stories array
                return (
                    <div className="story row" key={index}>
                        ...
                        <div className="col-10 details">
                            ...
                            <button onClick={() => navigate(`/stories/${each.story.replace(/(<([^>]+)>)/ig, '').replaceAll(' ', '-').replaceAll('?', '').substring(0, 45)}/${each._id}`)}>Read more</button>
                        </div>
                    </div>
                )
            })}
