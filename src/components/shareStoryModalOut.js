...
function ShareStoryModalOut({ story, openModal, setShareStoryModalOut }) {
  ...
  return (
    <Modal isOpen={openModal} onRequestClose={() => setShareStoryModalOut(false)} id="poll-share-modal" className={`${context.darkMode ? 'dm' : ""}`}>
      <i className="fas fa-times" onClick={() => setShareStoryModalOut(false)} />
